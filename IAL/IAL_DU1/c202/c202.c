...
void Stack_Init( Stack *stack ) {
	//bez zásobníku nie je čo inicializovať
	if(!stack){
...
bool Stack_IsEmpty( const Stack *stack ) {
	// ak sa vrchná položka zásobníku rovná prázdnej inicializovanej hodnote, zásobník je prádzny 
...
bool Stack_IsFull( const Stack *stack ) {
	// ak sa vrchná položka zásobníku rovná maximálnej hodnote zásboníku (čo je STACK_SIZE - 1), pseudodynamický zásobník je plný
...
void Stack_Top( const Stack *stack, char *dataPtr ) {
	// ak sa zavolá vrátenie najvrchnejšej položky, pre prádzny zásobník tak je to chybový stav, pri ktorom nie je čo vrátiť 
	if (Stack_IsEmpty(stack)){
		Stack_Error(SERR_TOP);
	}
	else{
		// else vráti vrchnú položku zásobníku cez cieľovú premennú
...
void Stack_Pop( Stack *stack ) {
	// ak stack nie je prázdny vráti vymaže najvrchnejšiu položku
	if (!Stack_IsEmpty(stack)){
		stack->topIndex--;
	}
	// to som nedal else na error lebo ak bude prádzny a nastane Pop tak ostane prázdny a nie je to chybový stav
}
...
void Stack_Push( Stack *stack, char data ) {
	// kontrola či nie je zásobník plný -> chybový stav
	if (Stack_IsFull(stack)){
		Stack_Error(SERR_PUSH);
	}
	else{
		// vloženie znaku do zásobníka
		stack->topIndex++;
